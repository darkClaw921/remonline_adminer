...
- `GET /subtabs/{subtab_id}/products` - получить товары листа
- `POST /subtabs/{subtab_id}/products` - добавить товары на лист (массив product_remonline_ids)
- `POST /subtabs/{subtab_id}/products/single` - добавить один товар на лист
...
- Используемые API эндпоинты:
  ...
  - `GET /api/v1/products/filtered?warehouse_ids=&stock_min=&...` — загрузка товаров с серверными фильтрами (при нажатии "Применить"). Параметр `name` поддерживает нечеткий поиск по названию товара и RemID. Параметры `sort_by/sort_order` передаются только для поддерживаемых ключей (name/category/price/total/wh_...).
